...
#[command(
    name = "sbx",
    version,
    about = "NovelAI Storyboard Studio — local-first template-driven workbench"
)]
struct Cli {
    #[command(subcommand)]
    cmd: Cmd,
    ...
    match cli.cmd {
        ...
        Cmd::Match {
            ...
            match server.match_templates(&intent, seed).unwrap() {
                Some(sel) => {
                    println!(
                        "\nPrimary: {} ({}) score={:.3} mode={:?} scene_adapt={}",
                        sel.primary.template_id,
                        sel.primary.title,
                        sel.primary.score,
                        sel.mode,
                        sel.needs_scene_adaptation
                    );
                    println!(
                        "breakdown: {}",
                        serde_json::to_string(&sel.primary.breakdown).unwrap()
                    );
        ...
        Cmd::Clone {
            ...
            let server = AppServer::open(&workspace).expect("open workspace");
            let state = server
                .clone_project(&template_id, title, seed)
                .expect("clone");
            println!(
                "project {} (v1) cloned from {} — title: {}",
                state.project_id, state.source_template_id, state.title
            );
        }
        Cmd::ListProjects { workspace } => {
            ...
            for p in server.db.list_projects().unwrap() {
                println!(
                    "{}  v{:<3} {:?}  {}",
                    p.id, p.current_version, p.status, p.title
                );
            }
        }
...
fn demo(server: &AppServer) {
    ...
    let selection = server
        ...
        .expect("match");
    println!(
        "【Template Match】 primary = {} ({}) score = {:.3}",
        selection.primary.template_id, selection.primary.title, selection.primary.score
    );

    // 2. clone
    let state = server
        .clone_project(
            selection.primary.template_id.as_str(),
            Some("夜の公園で犯される新人アイドル".into()),
            1234,
        )
        .expect("clone");
    let pid = state.project_id;
    println!(
        "cloned project {pid} ({} panels)",
        server.load_project_snapshot(&pid).unwrap().panel_count()
    );
    ...
    let (patch_id, report) = server
        .propose_patch(&pid, &proposal, Some("run_demo"))
        .expect("propose");
    ...
    println!(
        "committed v{} (parent v{}), preservation = {:.3}",
        outcome.new_version, outcome.parent_version, outcome.preservation_ratio
    );
