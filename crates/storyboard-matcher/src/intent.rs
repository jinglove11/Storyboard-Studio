...
pub fn parse_intent(input: &str, aliases: &SceneAliasTable) -> QueryIntent {
    ...
    if female + male > 0 {
        ...
        if female > 0 {
            q.character_roles.push(if female > 1 {
                "multiple_females".into()
            } else {
                "female_lead".into()
            });
        }
        if male > 0 {
            q.character_roles.push(if male > 1 {
                "multiple_males".into()
            } else {
                "single_male".into()
            });
    ...
    while let Some((idx, c)) = chars.next() {
        if c.is_ascii_digit() {
            ...
            let after = &lower[end..];
            if after.starts_with("格")
                || after.starts_with("张")
                || after.starts_with(" panels")
                || after.starts_with(" panels,")
            {
...
mod tests {
    ...
    fn aliases() -> SceneAliasTable {
        SceneAliasTable::from_pairs(BTreeMap::from([
            (
                "park".into(),
                vec!["park".into(), "公园".into(), "night park".into()],
            ),
            ("office".into(), vec!["office".into(), "办公室".into()]),
            (
                "school".into(),
                vec!["school".into(), "学校".into(), "教室".into()],
            ),
